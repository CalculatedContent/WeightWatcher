...
from weightwatcher import (DEFAULT_PARAMS, E_TPL, LAYER_TYPE, LOG_NORMAL, PL,
                           POWER_LAW, TPL, TRUNCATED_POWER_LAW)
...
class Test_VGG11(unittest.TestCase):
    ...
    def test_basic_columns(self):
        ...
        self.assertEqual(
            isinstance(details, pd.DataFrame), True, "details is a pandas DataFrame"
        )
        print(details)
        # TODO: add more columns ?
        for key in ["layer_id", "name", "M", "N"]:
            self.assertTrue(
                key in details.columns,
                "{} in details. Columns are {}".format(key, details.columns),
            )
    ...
    def test_model_layer_types_ww2x(self):
        ...
        self.assertEqual(
            denseCount, 3, "3 dense layers, but {} found".format(denseCount)
        )

        conv2DLayers = details[details.layer_type == str(LAYER_TYPE.CONV2D)]
        conv2DCount = len(conv2DLayers)
        self.assertEqual(
            conv2DCount, 8 * 9, "8*9 conv2D layers, but {} found".format(denseCount)
        )
    ...
    def test_all_layer_types(self):
        ...
        conv2DCount = len(conv2DLayers)
        self.assertEqual(
            conv2DCount, 8, "8 conv2D layers, but {} found".format(denseCount)
        )
    ...
    def test_filter_dense_layer_types(self):
        ...
        self.assertTrue(
            (denseLayers.N > 0).all(axis=None),
            "All {} dense layers have a non zero N".format(denseCount),
        )
        self.assertTrue(
            (denseLayers.M > 0).all(axis=None),
            "All {} dense layers have a non zero M".format(denseCount),
        )
        ...
        self.assertEqual(
            nonDenseCount,
            0,
            "Filter has No dense layers: {} found".format(nonDenseCount),
        )

        # Non Dense layers are NOT analyzed
        self.assertTrue(
            (nonDenseLayers.N == 0).all(axis=None),
            "All {} NON dense layers have a zero N".format(nonDenseCount),
        )
        self.assertTrue(
            (nonDenseLayers.M == 0).all(axis=None),
            "All {} NON dense layers have a zero M".format(nonDenseCount),
        )
    ...
    def test_negative_filter_layer_ids(self):
        ...
        denseCount = len(denseLayers)
        self.assertEqual(
            denseCount, 0, " no dense layers, but {} found".format(denseCount)
        )
    ...
    def test_filter_conv2D_layer_types(self):
        ...
        conv2DCount = len(conv2DLayers)
        self.assertEqual(
            conv2DCount, 8, "# conv2D layers: {} found".format(conv2DCount)
        )
        ...
        self.assertEqual(
            nonConv2DCount,
            0,
            "VGG11 has non conv2D layers: {} found".format(nonConv2DCount),
        )
    ...
    def test_compute_alphas(self):
        """Test that alphas are computed and values are within thresholds"""
        details = self.watcher.analyze(
            layers=[5], ww2x=True, randomize=False, plot=False, mp_fit=False
        )
    ...
    def test_get_details(self):
        ...
        self.assertEqual(
            ...
            "actual and expected details differ",
        )
    ...
    def test_randomize(self):
        ...
        details = self.watcher.analyze(
            layers=[25], randomize=False, plot=False, mp_fit=False
        )
        print(details.columns)
        ...
        print("----test_density_fit-----")
        details = self.watcher.analyze(
            layers=[25], randomize=True, plot=False, mp_fit=False
        )
    ...
    def test_density_fit(self):
        ...
        print("----test_density_fit-----")
        details = self.watcher.analyze(
            layers=[25], ww2x=True, randomize=False, plot=False, mp_fit=True
        )
        print(details.columns)
