...
class TestAnalyzeTraps(unittest.TestCase):
    ...
    def test_analyze_traps_trap_index_is_1_based(self):
        df = self.watcher.analyze_traps(plot=False, savefig=False, rng=1337)
        if len(df) == 0:
            self.skipTest("No traps detected in this environment")

        layer_id = int(df["layer_id"].iloc[0])
        layer_df = self.watcher.analyze_traps(layers=[layer_id], plot=False, savefig=False, rng=1337)
        if len(layer_df) == 0:
            self.skipTest("No traps detected for selected layer in this environment")

        trap_indices = sorted(layer_df["trap_index"].astype(int).tolist())
        self.assertEqual(min(trap_indices), 1)
        self.assertEqual(trap_indices, list(range(1, len(layer_df) + 1)))
