...
def best_dist(fit):
    ...
    R, p = fit.distribution_compare(
        distName, "stretched_exponential", normalized_ratio=True
    )
    if R < 0 and p <= 0.05:
        dist = "S_EXP"
        return dist
...
def marchenko_pastur_fun(x, Q, sigma=1):
    ...
    with warnings.catch_warnings():
        ...
        val = np.nan_to_num(
            (1 / (2 * np.pi * sigma * sigma * x * y)) * np.sqrt((b - x) * (x - a))
        )
...
def resid_mp(p, evals, Q, bw, allresid=True, num_spikes=0, debug=False):
    ...
    if Q == 1:
        ...
        if (
            (b < xde[np.where(yde == max(yde))][0])
            | (b > max(xde))
            | (a > xde[np.where(yde == max(yde))][0])
        ):
...
def fit_density(evals, Q, bw=0.1, sigma0=None):
    ...
    [sigma1], cov, infodict, mesg, ierr = optimize.leastsq(
        resid_mp, [sigma0], args=(to_fit, Q, bw), full_output=True
    )
...
def fit_density_with_range(evals, Q, bw=0.1, sigma_range=(slice(0.1, 1.25, 0.01),)):
    ...
    sigma_range = (
        slice(
            0.05 * sigma_scaling_factor,
            2.0 * sigma_scaling_factor,
            0.01 * sigma_scaling_factor,
        ),
    )
    #
    ###
    ...
    brute_output = optimize.brute(
        resid_mp, sigma_range, args=(to_fit, Q, bw, False), full_output=True
    )
