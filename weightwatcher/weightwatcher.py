...
def main():
    ...
    """
    print(
        "WeightWatcher command line support coming later. https://calculationconsulting.com"
    )
...
class WWLayer:
    ...
    def __init__(
        ...
        self.conv2d_count = (
            1  # reset by slice iterator for back compatability with ww2x
        )
        ...
        if self.framework == FRAMEWORK.PYTORCH:
            if hasattr(self.layer, "weight"):
                ...
                if self.the_type == LAYER_TYPE.CONV2D:
                    ...
                    logger.warn(
                        "pytorch layer: {}  type {} not found ".format(
                            str(self.layer), str(self.the_type)
                        )
                    )
            ...
            if self.the_type == LAYER_TYPE.CONV2D:
                ...
                logger.warn(
                    "keras layer: {} {}  type {} not found ".format(
                        self.layer.name, str(self.layer), str(self.the_type)
                    )
                )
    ...
    def set_weight_matrices(self, weights):  # , conv2d_fft=False, conv2d_norm=True):
        ...
        if (
            the_type == LAYER_TYPE.DENSE
            or the_type == LAYER_TYPE.CONV1D
            or the_type == LAYER_TYPE.EMBEDDING
        ):
            Wmats = [self.weights]
        ...
        # this is very slow with describe
            ...
            logger.info(
                "Layer id {}  unknown type {} layer  {}".format(
                    self.layer_id, the_type, type(self.layer)
                )
            )
    ...
    def get_conv2D_fft(self, W, n=32):
        ...
        if N + M >= imax + jmax:
            logger.debug(
                "[2,3] tensor shape detected: {}x{} (NxM), {}x{} (i,j)".format(
                    N, M, imax, jmax
                )
            )
            fft_axes = [2, 3]
            ...
            fft_axes = [0, 1]
            logger.debug(
                "[1,2] tensor shape detected: {}x{} (NxM), {}x{} (i,j)".format(
                    N, M, imax, jmax
                )
            )
    ...
    def conv2D_Wmats(self, Wtensor, channels=CHANNELS.UNKNOWN):
        ...
        if channels == CHANNELS.UNKNOWN:
            logger.debug(
                "channles UNKNOWN, detected {}".format(
                    self.channel_str(detected_channels)
                )
            )
        ...
        if detected_channels == channels:
            if channels == CHANNELS.LAST:
                logger.debug(
                    "Channels Last tensor shape: {}x{} (NxM), {}x{} (i,j)".format(
                        N, M, imax, jmax
                    )
                )
                for i in range(imax):
                ...
                # check this
                logger.debug(
                    "Channels First shape: {}x{} (NxM), {}x{} (i,j)".format(
                        N, M, imax, jmax
                    )
                )
                for i in range(imax):
            ...
            logger.warn(
                "warning, expected channels {},  detected channels {}".format(
                    self.channel_str(channels), self.channel_str(detected_channels)
                )
            )
            ...
            if detected_channels == CHANNELS.LAST:
                logger.debug(
                    "Flipping LAST to FIRST Channel, {}x{} ()x{}".format(
                        N, M, imax, jmax
                    )
                )
                for i in range(N):
                ...
                N, M, imax, jmax = imax, jmax, N, M
                logger.debug(
                    "Flipping FIRST to LAST Channel, {}x{} ()x{}".format(
                        N, M, imax, jmax
                    )
                )
                # check this
        ...
        logger.debug(
            "get_conv2D_Wmats N={} M={} rf= {} channels = {}".format(N, M, rf, channels)
        )
...
class ModelIterator:
    ...
    def __init__(self, model, params=DEFAULT_PARAMS):
        ...
        logger.debug(
            "FRAMEWORKS: KERAS = {}  PYTORCH = {} ONNX = {} UNKNOWN = {} ".format(
                FRAMEWORK.KERAS, FRAMEWORK.PYTORCH, FRAMEWORK.ONNX, FRAMEWORK.UNKNOWN
            )
        )
        logger.debug(
            "FIRST = {}  LAST = {} UNKNOWN = {} ".format(
                CHANNELS.FIRST, CHANNELS.LAST, CHANNELS.UNKNOWN
            )
        )
        ...
        logger.debug(
            "MODEL ITERATOR, framework = {}, channels = {} ".format(
                self.framework, self.channels
            )
        )
...
class WWLayerIterator(ModelIterator):
    ...
    def apply_filters(self, ww_layer):
        ...
        if self.filter_types is not None and len(self.filter_types) > 0:
            if ww_layer.the_type in self.filter_types:
                logger.debug(
                    "keeping layer {} {} with type {} ".format(
                        ww_layer.layer_id, ww_layer.name, str(ww_layer.the_type)
                    )
                )
                ww_layer.skipped = False
            else:
                logger.debug(
                    "skipping layer {} {} with type {} ".format(
                        ww_layer.layer_id, ww_layer.name, str(ww_layer.the_type)
                    )
                )
        ...
        if self.filter_ids is not None and len(self.filter_ids) > 0:
            ...
            if np.min(self.filter_ids) > 0:
                if ww_layer.layer_id in self.filter_ids:
                    logger.debug(
                        "keeping layer {} {} by id".format(
                            ww_layer.layer_id, ww_layer.name
                        )
                    )
                    ww_layer.skipped = False
                else:
                    logger.debug(
                        "skipping layer {} {} by id".format(
                            ww_layer.layer_id, ww_layer.name
                        )
                    )
                    ww_layer.skipped = True
    ...
    def layer_supported(self, ww_layer):
        ...
        if ww_layer.skipped:
            ...
            logger.debug(
                "layer not supported: Layer {} {} has no weights".format(layer_id, name)
            )
            return False

        elif the_type is LAYER_TYPE.UNKNOWN:
            logger.debug(
                "layer not supported: Layer {} {} type {} unknown".format(
                    layer_id, name, the_type
                )
            )
            return False
            ...
            logger.debug(
                "layer not supported: Layer {} {} type {} not supported".format(
                    layer_id, name, the_type
                )
            )
            return False
            ...
            logger.debug(
                "layer not supported: Layer {} {}: num_evals {} <  min_evals {}".format(
                    layer_id, name, M, min_evals
                )
            )
            return False
            ...
            logger.debug(
                "layer not supported: Layer {} {}: num_evals {} > max_evals {}".format(
                    layer_id, name, N, max_evals
                )
            )
            return False
    ...
    def ww_intralayer_iter_(self):
        ...
        def align_mats(W0, W1):
            ...
            if len(shapes) > 0:
                ...
                if not shared_dim:
                    logger.warning(
                        "Can not align W0={} with W1={}".format(W0.shape, W1.shape)
                    )
                    return W0, W1
...
class WeightWatcher(object):
    ...
    def load_model(self, model):
        ...
        if isinstance(model, str):
            if os.path.isfile(model):
                ...
                logger.error(
                    "Loading model from file '{}': file not found".format(model)
                )
        return res
    ...
    def distances(self, model_1, model_2):
        ...
        details = pd.DataFrame(
            columns=["layer_id", "name", "delta_W", "delta_b", "W_shape", "b_shape"]
        )
        data = {}
    ...
    def combined_eigenvalues(self, Wmats, N, M, n_comp, params):
        ...
        for W in Wmats:
            ...
            W = W.astype(float)
            logger.debug(
                "Running full SVD:  W.shape={}  n_comp = {}".format(W.shape, n_comp)
            )
    ...
    def apply_esd(self, ww_layer, params=DEFAULT_PARAMS):
        ...
        evals, sv_max, rank_loss = self.combined_eigenvalues(
            Wmats, N, M, n_comp, params
        )
    ...
    def make_layer_iterator(self, model=None, layers=[], params=DEFAULT_PARAMS):
        ...
        if stacked:
            ...
            layer_iterator = WWStackedLayerIterator(
                model, filters=layers, params=params
            )
        elif intra:
    ...
    def analyze(
        ...
        if min_size or max_size:
            logger.warn(
                "min_size and max_size options changed to min_evals, max_evals, ignored for now"
            )
        ...
        layer_iterator = self.make_layer_iterator(
        ...
        for ww_layer in layer_iterator:
            if not ww_layer.skipped and ww_layer.has_weights:
                logger.debug(
                    "LAYER: {} {}  : {}".format(
                        ww_layer.layer_id, ww_layer.the_type, type(ww_layer.layer)
                    )
                )
                ...
                if ww_layer.evals is not None:
                    ...
                    if params["deltaEs"] and params["plot"]:
                        logger.debug(
                            "Computing and Plotting Deltas: {} {} ".format(
                                ww_layer.layer_id, ww_layer.name
                            )
                        )
                    ...
                    if params["vectors"]:  # and params['plot']:
                        logger.debug(
                            "Computing and Plotting Vector Localization Metrics: {} {} ".format(
                                ww_layer.layer_id, ww_layer.name
                            )
                        )
                    ...
                    if params["randomize"]:  # params['mp_fit']:
                        logger.debug(
                            "Randomizing Layer: {} {} ".format(
                                ww_layer.layer_id, ww_layer.name
                            )
                        )
                        self.apply_random_esd(ww_layer, params)
                        logger.debug(
                            "MP Fitting Random layer: {} {} ".format(
                                ww_layer.layer_id, ww_layer.name
                            )
                        )
                        ...
                        if params["deltaEs"] and params["plot"]:
                            logger.debug(
                                "Cpmputing and Plotting Deltas: {} {} ".format(
                                    ww_layer.layer_id, ww_layer.name
                                )
                            )
                            self.apply_plot_deltaEs(
                                ww_layer, random=True, params=params
                            )
    ...
    def describe(
        ...
        for ww_layer in layer_iterator:
            if not ww_layer.skipped and ww_layer.has_weights:
                logger.debug(
                    "LAYER TYPE: {} {}  layer type {}".format(
                        ww_layer.layer_id, ww_layer.the_type, type(ww_layer.layer)
                    )
                )
                logger.debug(
                    "weights shape : {}  max size {}".format(
                        ww_layer.weights.shape, params["max_evals"]
                    )
                )
                if ww2x:
    ...
    def valid_params(self, params):
        ...
        if filters is not None:
            ...
            if len(filter_ids) > 0:
                if np.max(filter_ids) > 0 and np.min(filter_ids) < 0:
                    logger.warn(
                        "layer filter ids must be all > 0 or < 0: {}".format(filter_ids)
                    )
                    valid = False
        ...
        if fix_fingers:
            if fix_fingers not in [XMIN_PEAK, CLIP_XMAX]:
                logger.warning(
                    "Unknown how to fix fingers {}, deactivating".format(fix_fingers)
                )
            else:
        ...
        if intra:
            if params["randomize"] or params["vectors"]:
                logger.fatal(
                    "Can not set intra=True with randomize=Ttue or vectors=True at this time"
                )
    ...
    def glorot_norm_check(self, W, N, M, rf_size, lower=0.5, upper=1.5):
    ...
    def random_eigenvalues(self, Wmats, n_comp, num_replicas=1, params=DEFAULT_PARAMS):
        ...
        logger.info(
            "generating {} replicas for each W of the random eigenvalues".format(
                num_replicas
            )
        )
    ...
    def plot_random_esd(self, ww_layer, params=DEFAULT_PARAMS):
        ...
        plt.hist(
            np.log10(nonzero_evals), bins=100, density=True, color="g", label="original"
        )
        plt.hist(
            np.log10(nonzero_rand_evals),
            bins=100,
            density=True,
            color="r",
            label="random",
        ...
        title = "Layer {} {}: Log10 ESD & Random ESD".format(
            ww_layer.layer_id, ww_layer.name
        )
        plt.title(title)
    ...
    def fit_powerlaw(
        ...
        if sample and sample_size is None:
            logger.info(
                "setting sample size to default MAX_NUM_EVALS={}".format(MAX_NUM_EVALS)
            )
        ...
        if sample and num_evals > sample_size:
            ...
            logger.info(
                "chosing {} eigenvalues from {} ".format(sample_size, len(evals))
            )
        ...
        if fix_fingers == XMIN_PEAK:
            ...
            try:
                nz_evals = evals[evals > thresh]
                fit = powerlaw.Fit(
                    nz_evals, xmax=xmax, verbose=False, distribution=distribution
                )
                status = SUCCESS
            except ValueError:
                status = FAILED
            ...
            try:
                fit = powerlaw.Fit(
                    evals, xmin=xmin, verbose=False, distribution=distribution
                )
                status = SUCCESS
            except ValueError:
                status = FAILED
        ...
        if status == FAILED:
            ...
            for dist in all_dists[1:]:
                R, p = fit.distribution_compare(
                    dist, TRUNCATED_POWER_LAW, normalized_ratio=True
                )
                ...
                if R > 0.1 and p > 0.05:
                    ...
                    Rs.append(R)
                    logger.debug(
                        "compare dist={} R={:0.3f} p={:0.3f}".format(dist, R, p)
                    )
    ...
    def get_ESD(self, model=None, layer=None, random=False, params=DEFAULT_PARAMS):
        ...
        if type(layer) is int and layer not in layer_ids:
            logger.error(
                "Can not find layer id {} in valid layer_ids {}".format(
                    layer, layer_ids
                )
            )
            return []
            ...
            logger.error(
                "Can not find layer name {} in valid layer_names {}".format(
                    layer, layer_names
                )
            )
            return []
        ...
        if esd is None or len(esd) == 0:
            logger.warn(
                "No eigenvalues found for {} {}".format(
                    ww_layer.layer_id, ww_layer.name
                )
            )
            ...
            logger.debug(
                "Found {} eiganvalues for {} {}".format(
                    len(esd), ww_layer.layer_id, ww_layer.name
                )
            )
    ...
    def mp_fit(
        ...
        logger.debug(
            "bulk_max = {:0.3f}, bulk_max_TW = {:0.3f} ".format(bulk_max, bulk_max_TW)
        )
        ...
        if Q == 1.0:
            ...
            if plot:
                plot_density(
                    to_plot, Q=Q, sigma=s1, method="MP", color=color, cutoff=bulk_max_TW
                )  # , scale=Wscale)
    ...
    def smooth_W(self, W, n_comp):
        ...
        logger.debug(
            "smoothed W {} -> {} n_comp={}".format(W.shape, smoothed_W.shape, n_comp)
        )
    ...
    def apply_svd_smoothing(self, ww_layer, params=DEFAULT_PARAMS):
        ...
        if channels == CHANNELS.UNKNOWN:
            log.error(
                "Sorry, SVDSmoothing does not understand the channels for this layer, stopping "
            )
            return
        ...
        logger.info(
            "apply truncated SVD on Layer {} {}, with nsmooth={},  keeping ncomp={} out of {}. of the singular vectors".format(
                layer_id, layer_name, num_smooth, n_comp, ww_layer.num_components
            )
        )
        ...
        logger.info(
            "LAYER TYPE  {} out of {} {} {} ".format(
                layer_type, LAYER_TYPE.DENSE, LAYER_TYPE.CONV1D, LAYER_TYPE.EMBEDDING
            )
        )
        ...
        if layer_type in [LAYER_TYPE.DENSE, LAYER_TYPE.CONV1D, LAYER_TYPE.EMBEDDING]:
            if params["smooth"] == RMT:
                ...
                logger.warning(
                    "Not smoothing {} {}, ncomp=0".format(layer_id, layer_name)
                )
                new_W = old_W
        ...
        # if not ww2x, then we need to divide num_smooth / rf
            ...
            if new_B is not None:
                logger.warn(
                    "Something went wrong, Biases found for Conv2D layer, layer {} {} ".format(
                        layer_id, layer_name
                    )
                )
            ...
            if channels == CHANNELS.FIRST:
                ...
                if rf != i_max * j_max:
                    logger.warn(
                        "Channels FIRST not processed correctly W_slice.shape {}, rf={} ?".format(
                            new_W.shape, rf
                        )
                    )
                ...
                for i in range(i_max):
                    for j in range(j_max):
                        if num_smooth > 0:
                            logger.debug(
                                "Keeping top {} singular values".format(num_smooth)
                            )
                            new_W[i, j, :, :] = self.smooth_W(
                                old_W[i, j, :, :], num_smooth
                            )
                        elif num_smooth < 0:
                            logger.debug(
                                "Chomping off top {} singular values".format(
                                    -num_smooth
                                )
                            )
                            new_W[i, j, :, :] = self.smooth_W_alt(
                                old_W[i, j, :, :], num_smooth
                            )
                        else:
                            new_W[i, j, :, :] = old_W[i, j, :, :]
            ...
            # [N,M,k,k]
                ...
                if rf != i_max * j_max:
                    logger.warn(
                        "Channels LAST not processed correctly W_slice.shape {}, rf={} ?".format(
                            new_W.shape, rf
                        )
                    )
                ...
                logger.warn(
                    "Something went wrong, Channels not defined or detected for Conv2D layer, layer {} {} skipped ".format(
                        layer_id, layer_name
                    )
                )
            ...
            logger.warn(
                "Something went wrong,UNKNOWN layer {} {} skipped , type={}".format(
    ...
    def replace_layer_weights(self, framework, idx, layer, W, B=None):
        ...
        # See; https://github.com/onnx/onnx/issues/2978
            ...
            logger.debug(
                "Layer {} skipped, Layer Type {} not supported".format(
                    layer_id, the_type
                )
            )
